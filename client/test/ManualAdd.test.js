...
import ManualAdd from '../src/components/Application/ManualAdd'
...
function testManualFields() {
    const fields = mount((
        <ManualAdd config={startProps.config} trip={startProps.trip}/>
    ));

    let actual = [];
    fields.find('InputGroupAddon').map((element) => actual.push(element.text().charAt(0).toLowerCase() +
...
const upload = mount(<ManualAdd config={startProps.config} trip={startProps.trip} updateTrip={updateMyTrip}/>);
